...
function viewHome() {
  ...
  var phases = course.groups.map(function (g, i) {
    ...
    return '<div class="card">' +
      '<div class="card-top"><div class="card-n">' + (i === 0 ? icon("compass") : String(i - 1)) + "</div>" +
      '<h3><a href="#/' + first.slug + '">' + esc(g.title.replace(/^Giai đoạn \d+ — /, "")) + "</a></h3></div>" +
  ...
  var cases = D.nav.slice(1, 2).map(function (sec) {
    ...
    return '<a class="card" href="#/' + first.slug + '">' +
      ...
      "<p>" + esc(sec.sub) + " — mỗi đồ án phải có: tài liệu thiết kế 11 mục, hệ thống chạy được, " +
      "benchmark tìm nút thắt, <b>fault injection</b>, ADR và tự chấm.</p>" +
      '<div class="card-foot"><div class="bar"><i style="width:' + (gs.p * 100).toFixed(0) +
      '%"></i></div><b>' + gs.n + "/" + gs.t + "</b></div></a>";
  }).join("");

  var refs = D.nav[2].groups[0].items.map(function (id) {
    var d = DOCS[id];
  ...
  $("#main").innerHTML =
    '<div class="home">' +
      '<div class="hero">' +
        "<h1>Học <u>System Design</u> từ số 0 đến kiến trúc production</h1>" +
        "<p>58 bài giảng tiếng Việt dẫn bạn từ “viết đúng” sang “thiết kế được”: ước lượng năng lực, " +
        "dữ liệu và nhất quán, hệ phân tán, vận hành production — kèm <b>6 đồ án bắt buộc</b> " +
        "phải xây, phá và đo thật.</p>" +
        '<div class="hero-cta">' +
          '<a class="btn btn-p" href="#/' + DOCS[ORDER[0]].slug + '">' + icon("right") + "Bắt đầu học</a>" +
          '<a class="btn btn-s" href="#/bai/00-de-cuong">Xem đề cương</a>' +
          '<a class="btn btn-s" href="#/do-an/do-an">' + icon("layers") + "Sáu đồ án</a>" +
        "</div>" +
      "</div>" +
      ...
      '<div class="sec-h"><h2>Sáu đồ án bắt buộc</h2><span>khoá học hoàn thành bằng việc XÂY</span></div>' +
  ...
  document.title = "Học System Design — Từ số 0 đến kiến trúc production";
...
function viewDoc(doc, anchor) {
  ...
  var m = doc.meta || {};
  var chips = [];
  if (m.no) chips.push('<span class="chip ac">Bài ' + m.no + "/" + D.stats.lessons + "</span>");
