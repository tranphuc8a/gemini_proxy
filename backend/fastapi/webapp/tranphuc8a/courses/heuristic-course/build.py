...
TREE = [
    {
        ...
        "sub": "Từ cơ bản đến chuyên sâu · 23 bài",
        "icon": "compass",
        "groups": [
            {
                "title": "Bắt đầu",
...
def main():
    ...
    with open(OUT, "w", encoding="utf-8") as fh:
        fh.write("/* Tu dong sinh boi build.py — dung sua tay. */\n")
        fh.write("window.COURSE = ")
        json.dump(payload, fh, ensure_ascii=False, separators=(",", ":"))
        fh.write(";\n")

    size = os.path.getsize(OUT)
    s = payload["stats"]
    print("da ghi %s" % os.path.relpath(OUT, HERE))
    print("  %d tai lieu, %d tu, ~%d gio doc, %.1f MB"
          % (s["files"], s["words"], round(s["minutes"] / 60), size / 1048576.0))
